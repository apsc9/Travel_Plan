onload = function () {
    ...
    function createData(){
        ...
        let edges = [];
        for(let i=2;i<=V;i++){
            let neigh = i - Math.floor(Math.random()*Math.min(i-1,3)+1); // Picks a neighbour from i-3 to i-1 so that the close ones are not chosen repeatedly
    ...
    function solveData() {
        ...
        let new_edges = [];
        if(plane!==0){
            new_edges.push({arrows: { to: { enabled: true}}, from: p1+1, to: p2+1, color: 'black',label: String(plane)});
